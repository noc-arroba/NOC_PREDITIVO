...
// Filtro HARD: apenas OLTs ativas | Validação de coordenadas GPS
...
// Bounding box de Campos dos Goytacazes e arredores (com margem)
const CAMPOS_LAT_MIN = -22.05;
const CAMPOS_LAT_MAX = -21.55;
const CAMPOS_LON_MIN = -41.50;
const CAMPOS_LON_MAX = -41.05;

// Valida se a coordenada está dentro da região de Campos dos Goytacazes
function coordValida(lat, lon) {
  ...
  // Rejeitar valor default do sistema (-21.0, -48.0)
...
async function fetchAllFTTH() {
  ...
  let stats = { totalClientes: 0, online: 0, offline: 0, outros: 0, transmissoresUnicos: 0, pons: 0, caixas: 0,
    sinalBom: 0, sinalAtencao: 0, sinalCritico: 0, comSinal: 0, ctosComCoord: 0 };
  ...
  for (const c of clientes) {
    ...
    // Acumular coordenadas da CTO (apenas coordenadas VÁLIDAS)
    if (idCaixa && idCaixa !== '0') {
      ...
      // FILTRO: só acumular se a coordenada for válida (dentro de Campos)
  ...
  // Monta array de CTOs com coordenadas VÁLIDAS (filtra as que não têm nenhuma coordenada válida)
  ...
  return {
    ...
    transmissoresUnicos, hierarquia, ctos
  };
}
